...
<<<<<<< HEAD
    ...
    maintenance_gap = 1 - (min(max(safe_float(form_values.get("maintenance_score"), 70.0), 0.0), 100.0) / 100.0)
    occupancy = min(max(safe_float(form_values.get("occupancy_load"), 700.0), 0.0), 6000.0) / 6000.0
    drainage_factor = {"Good": 0.1, "Moderate": 0.45, "Poor": 0.9}.get(str(form_values.get("drainage_quality", "Moderate")), 0.45)
    material_factor = {"RCC": 0.2, "Steel": 0.28, "Brick Masonry": 0.65}.get(str(form_values.get("construction_material", "RCC")), 0.35)
    foundation_factor = {
        "Pile Foundation": 0.12,
        "Raft Foundation": 0.18,
        "Isolated Footing": 0.38,
        "Strip Footing": 0.58,
    }.get(str(form_values.get("foundation_type", "Isolated Footing")), 0.35)
    ...
    score = (
        ...
        + soil_capacity_gap * 0.05
        + maintenance_gap * 0.06
        + occupancy * 0.03
...
<<<<<<< HEAD
    ...
    context["drainage_quality"] = str(asset.get("drainage_quality", "Moderate"))
        ...
        [
            {
                ...
                "water_level_m": safe_float(form_values["water_level_m"], defaults["water_level_m"]),
                "soil_type": str(form_values.get("soil_type", defaults["soil_type"])),
                "building_age_years": safe_float(form_values["building_age_years"], defaults["building_age_years"]),
                "floors": safe_float(form_values["floors"], defaults["floors"]),
                "building_height_m": safe_float(form_values["building_height_m"], defaults["building_height_m"]),
                "distance_to_river_km": safe_float(form_values["distance_to_river_km"], defaults["distance_to_river_km"]),
                "construction_material": str(form_values.get("construction_material", defaults["construction_material"])),
                "foundation_type": str(form_values.get("foundation_type", defaults["foundation_type"])),
                "drainage_quality": str(form_values.get("drainage_quality", defaults["drainage_quality"])),
                "soil_bearing_capacity_kpa": safe_float(
                    form_values["soil_bearing_capacity_kpa"],
                    defaults["soil_bearing_capacity_kpa"],
                ),
                "maintenance_score": safe_float(form_values["maintenance_score"], defaults["maintenance_score"]),
                "occupancy_load": safe_float(form_values["occupancy_load"], defaults["occupancy_load"]),
            }
        ]
    )
...
<<<<<<< HEAD
        ...
        f"Construction Material: {form_values['construction_material']}",
        f"Foundation Type: {form_values['foundation_type']}",
        f"Drainage Quality: {form_values['drainage_quality']}",
